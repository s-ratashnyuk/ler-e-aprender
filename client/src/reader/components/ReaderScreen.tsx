...
const ReaderScreen = (): JSX.Element => {
  const dispatch = useAppDispatch();
  const selectActiveBookId = (state: rootState): string => state.reader.activeBookId;
  const selectProgressByBook = (state: rootState): Record<string, number> =>
    state.reader.progressByBook;
  ...
  const getTokens = (): textToken[] => splitTextToTokens(storyText);
  ...
  useLayoutEffect(syncScrollPosition, [savedProgress, tokens]);
  ...
  const handleContainerClickCapture = (event: MouseEvent<HTMLDivElement>): void => {
  ...
  const handleTokenClick = async (tokenIndex: number): Promise<void> => {
    if (popupState.isOpen) {
      return;
    }
  ...
  const renderToken = (token: textToken): JSX.Element => {
    ...
    return (
      <span key={token.index} className={className} onClick={() => handleTokenClick(token.index)}>
        {token.text}
      </span>
    );
  };
